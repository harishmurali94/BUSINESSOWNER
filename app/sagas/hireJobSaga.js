...
import * as hireForJobAction from 'app/actions/hireForJobAction';
import * as loaderAction from 'app/actions/loaderActions';
import getHireJob from 'app/api/methods/getHireJob';
import * as navigateActions from '../actions/navigationActions';

// Our worker Saga that logins the user
export default function* hireJobAsync(action) {
  ...
  try{
    const response = yield call(getHireJob, action.params,userToken);
    if(response.Status ===  "Success"){
        yield put(hireForJobAction.responseHire("Success"));
        yield call(navigateActions.navigateToJobList());
        yield put(loaderAction.disableLoader({}));
    }else{
        yield put(hireForJobAction.failedHire("Failed"));
        yield put(loaderAction.disableLoader({}));
        yield delay(100)
